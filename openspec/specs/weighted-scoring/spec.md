...
cap, (3) apply hard constraints `min_context`, `max_input_price`, `requires`,
...
value on any positively-weighted metric. Provider scope acts as a hard pre-filter, the same
tier as `min_context`/`max_input_price`/`requires`/`exclude_family_of` — never a soft
...
"no model available" reason path rather than raising a hard error or panicking.
